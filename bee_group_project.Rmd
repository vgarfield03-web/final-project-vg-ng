...
nlcd_classes <- tibble::tibble(
  landcover = c(81,24), 
  landcover_name = c("Pasture/Hay", "Developed High Intensity" )
)
...
nlcd_counties_df$urban_rural <- ifelse(
  nlcd_counties_df$Annual_NLCD_LndCov_2020_CU_C1V1 %in% c(24),
  "Urban",
  "Rural"
)
...
nlcd_df$urban_rural <- ifelse(
  nlcd_df$Annual_NLCD_LndCov_2020_CU_C1V1 %in% c(24),
  "Urban",
  "Rural"
)
```
...
```{r}
oba_counties_df <- oba_sf_counties

#This merges the genus and species columns to display the full name of the bees
oba_counties_df$BeeSpecies <- paste(oba_counties_df$genus, oba_counties_df$specificEpithet, sep = " ")
sort(unique(oba_counties_df$BeeSpecies), decreasing = FALSE)

oba_counties_df$PlantSpecies <- paste(oba_counties_df$genusPlant, oba_counties_df$speciesPlant, sep = " ")
sort(unique(oba_counties_df$PlantSpecies), decreasing = FALSE)

```

```{r}
#For this project, we will be focusing on bee and plant populations in these four counties

target <- c("Clackamas", "Multnomah", "Sherman", "Wheeler")

#This subsets the 

subsetted_oba_for_bees <- oba_counties_df %>%
  group_by(BeeSpecies, county) %>%
  summarize(value = n()) %>%
...
subsetted_oba_for_plants <- oba_counties_df %>%
  group_by(PlantSpecies, county) %>%
  summarize(value = n()) %>%
  filter(county %in% target) %>%
  filter(value < 100) %>%
  arrange(desc(value)) %>%
...
subsetted_oba_for_bees <- sf::st_drop_geometry(subsetted_oba_for_bees)
glimpse(subsetted_oba_for_bees)

subsetted_oba_for_plants <- sf::st_drop_geometry(subsetted_oba_for_plants)
glimpse(subsetted_oba_for_plants)
```
